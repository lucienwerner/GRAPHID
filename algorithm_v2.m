function [Ycorrect,errors] = algorithm_v2(I,V,Y)
    %{
    ...
    errors(1) = norm(Y - Ycorrect,'fro')/(N^2); %normalized frobenius "per entry"
    errors(2) = max(max(abs(Y - Ycorrect))); %largest per-entry error
    errors(3) = topology_error(Y,Ycorrect); %number of topology errors, thresh = 1e-5
    % print()
